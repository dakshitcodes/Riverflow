...
const Page = async ({
    searchParams: searchParamsPromise, // ✅ 1. Accept searchParams as a Promise wrapper
}: {
    // ✅ 2. Type definition explicitly defined as a Promise for Next.js 15/16 compliance
    searchParams: Promise<{ page?: string; tag?: string; search?: string }>; 
}) => {
    // ✅ 3. Unpack/unwrap the parameters asynchronously before reading properties
    const searchParams = await searchParamsPromise;
    
    // Assign standard default fallback value cleanly
    ...
    const processedDocuments: QuestionCardDocument[] = await Promise.all(
        questions.documents.map(async doc => {
            ...
            } as unknown as QuestionCardDocument;
        })
    );
