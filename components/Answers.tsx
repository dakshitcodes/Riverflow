"use client";

import { ID, Models } from "appwrite";
import React from "react";
import VoteButtons from "./VoteButtons";
import { useAuthStore } from "@/store/Auth";
import { avatars } from "@/models/client/config";
import RTE, { MarkdownPreview } from "./RTE";
import Comments, { CommentDocument } from "./Comments";
import slugify from "@/utils/slugify";
import Link from "next/link";
import { IconTrash } from "@tabler/icons-react";

export interface CustomUser extends Models.User<Models.Preferences> {
    reputation: number;
}

export interface AnswerDocument extends Models.Document {
    content: string;
    questionId: string;
    authorId: string;
    upvotesDocuments: Models.DocumentList<Models.Document>;
    downvotesDocuments: Models.DocumentList<Models.Document>;
    comments: Models.DocumentList<CommentDocument>; 
    author: {
        $id: string;
        name: string;
        reputation: number;
    };
}

const Answers = ({
    answers: _answers,
    questionId,
}: {
    answers: Models.DocumentList<AnswerDocument>;
    questionId: string;
}) => {
    const [answers, setAnswers] = React.useState<Models.DocumentList<AnswerDocument>>(_answers);
    const [newAnswer, setNewAnswer] = React.useState("");
    const { user } = useAuthStore();

    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!newAnswer || !user) return;

        // Cast local user to access your custom fields safely
        const appUser = user as unknown as CustomUser;

        try {
            const response = await fetch("/api/answer", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    questionId: questionId,
                    answer: newAnswer,
                    authorId: appUser.$id,
                }),
            });

            const data = await response.json();
            if (!response.ok) throw data;

            const optimisticAnswer: AnswerDocument = {
                $id: data.$id || ID.unique(),
                $collectionId: data.$collectionId || "",
                $databaseId: data.$databaseId || "",
                $createdAt: data.$createdAt || new Date().toISOString(),
                $updatedAt: data.$updatedAt || new Date().toISOString(),
                $permissions: data.$permissions || [],
                $sequence: data.$sequence || "",
                content: newAnswer,
                questionId: questionId,
                authorId: appUser.$id,
                author: {
                    $id: appUser.$id,
                    name: appUser.name || "Anonymous User",
                    reputation: appUser.reputation || 0, // ✅ Error Fixed
                },
                upvotesDocuments: { documents: [], total: 0 },
                downvotesDocuments: { documents: [], total: 0 },
                comments: { documents: [], total: 0 },
            };

            setNewAnswer(() => "");
            setAnswers(prev => ({
                total: prev.total + 1,
                documents: [optimisticAnswer, ...prev.documents],
            }));
        } catch (error: any) {
            window.alert(error?.message || "Error creating answer");
        }
    };

    const deleteAnswer = async (answerId: string) => {
        try {
            const response = await fetch("/api/answer", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ answerId }),
            });

            const data = await response.json();
            if (!response.ok) throw data;

            setAnswers(prev => ({
                total: prev.total - 1,
                documents: prev.documents.filter(answer => answer.$id !== answerId),
            }));
        } catch (error: any) {
            window.alert(error?.message || "Error deleting answer");
        }
    };

    return (
        <>
            <h2 className="mb-4 text-xl">{answers.total} Answers</h2>
            {answers.documents.map((answer: AnswerDocument) => {
                console.log("Answer Content:", answer.content);
                // Clean empty image markdown tags ![]() or ![image]() to avoid browser errors
                const cleanContent = answer.content?.replace(/!\[.*?\]\(\s*\)/g, "") || "";
                return (
                    <div key={answer.$id} className="flex gap-4">
                        <div className="flex shrink-0 flex-col items-center gap-4">
                            <VoteButtons
                                type="answer"
                                id={answer.$id}
                                upvotes={answer.upvotesDocuments}
                                downvotes={answer.downvotesDocuments}
                            />
                            {user?.$id === answer.authorId ? (
                                <button
                                    className="flex h-10 w-10 items-center justify-center rounded-full border border-red-500 p-1 text-red-500 duration-200 hover:bg-red-500/10"
                                    onClick={() => deleteAnswer(answer.$id)}
                                    type="button"
                                >
                                    <IconTrash className="h-4 w-4" />
                                </button>
                            ) : null}
                        </div>
                        <div className="w-full overflow-auto">
                            <MarkdownPreview className="rounded-xl p-4" source={cleanContent} />
                            <div className="mt-4 flex items-center justify-end gap-1">
                            <picture>
                                <img
                                    src={avatars.getInitials(answer.author?.name || "U", 36, 36)}
                                    alt={answer.author?.name || "User Avatar"}
                                    className="rounded-lg"
                                />
                            </picture>
                            <div className="block leading-tight">
                                <Link
                                    href={`/users/${answer.author?.$id}/${slugify(answer.author?.name || "user")}`}
                                    className="text-orange-500 hover:text-orange-600"
                                >
                                    {answer.author?.name || "Anonymous User"}
                                </Link>
                                <p>
                                    <strong>{answer.author?.reputation ?? 0}</strong>
                                </p>
                            </div>
                        </div>
                        <Comments
                            comments={answer.comments}
                            className="mt-4"
                            type="answer"
                            typeId={answer.$id}
                        />
                        <hr className="my-4 border-white/40" />
                    </div>
                </div>
            );
        })}
            <hr className="my-4 border-white/40" />
            <form onSubmit={handleSubmit} className="space-y-2">
                <h2 className="mb-4 text-xl">Your Answer</h2>
                <RTE value={newAnswer} onChange={value => setNewAnswer(() => value || "")} />
                <button className="shrink-0 rounded bg-orange-500 px-4 py-2 font-bold text-white hover:bg-orange-600" type="submit">
                    Post Your Answer
                </button>
            </form>
        </>
    );
};

export default Answers;